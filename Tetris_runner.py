...
import time
import winsound
from combo_path_finder import findpath, get_piece_queue
import pygame as pg
...
#                            for _ in range(Tetris.GAME_HEIGHT // Tetris.BOX_SIZE)]
    ...
    def update_piece(self):
        ...
        if self.first:
            ...
            print("drop")
            delay=randint(80,120)
            self.canvas.after(delay, self.play_sound)
            self.canvas.after(delay+80,self.autodrop)
    ...
    def start(self):
        ...
        while(i<10):
            ...
            if not path:
                ...
                if i==9:
                    piece_queue=get_piece_queue()   #계속 못찾으면 새로 queue를 받음
                    i=0
            else:
                break;
            i+=1
